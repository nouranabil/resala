module CompactFlags
  ...
  def compact_flags(mapping)
    ...
    flags.each do |flag|
      ...
      # scopes to be used in conditions
      scope "#{flag}".pluralize.to_sym, :conditions=>"(#{@@store_columns[flag]} & #{@@flags[flag]}) > 0"
      scope "not_#{flag}".pluralize.to_sym, :conditions=>"(#{@@store_columns[flag]} & #{@@flags[flag]}) = 0"
    end
